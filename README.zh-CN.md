...
为了获得最流畅的星际冒险与拍卖行交易体验，请先确认以下前置准备：

### 1. 建议游玩环境（二选一最佳）
* **💻 电脑端 (PC / Mac) [最强烈推荐]**：
  * 使用 Chrome / Brave / Edge 等主流浏览器，并安装 **[MetaMask 浏览器插件 (Extension)](https://metamask.io/)**。
  * 点击游戏右上角“连接钱包”，插件将自动弹出并引导自动添加与切换至 **0G Galileo Testnet**。
* **📱 手机端 (iOS / Android) [移动端最佳解]**：
  * 请打开 **MetaMask 手机 App** ➔ 点击底部导航栏的 **“浏览器”图标** ➔ 在地址栏输入线上体验网址：`https://idlecoll.yayayayaya.xyz`。
  * ⚠️ *注意：请务必在 MetaMask 内置浏览器中打开，可享有“零跳转、签名弹窗即时浮现、100% 交易成功率”的原生手游体验！*

### 2. ⚠️ 钱包连接环境与 SDK 限制说明 (为何不使用外部 SDK)
* **为何不支持手机外部浏览器第三方 SDK（如 MetaMask SDK / WalletConnect / 二维码扫码）？**
  * **自定义 EVM 链兼容性限制**：0G Galileo 测试网（Chain ID: `16602`）属于自定义 EVM 网络。在手机外部浏览器（Safari 或 Mobile Chrome）使用第三方 SDK 时，受限于手机系统严格的后台机制与跨 App 切换（Deep Link），极易发生 **RPC 握手超时、网络自动切换失效、交易签名广播中断** 等不可抗力的交易卡死问题。
  * **全面回归原生 Web3 Provider (EIP-1193 / EIP-3326)**：为保障玩家资产安全与交易流畅度，本项目不采用外部第三方 SDK，全面使用标准原生 Provider 协议：
    * **💻 电脑端**：限定使用 **MetaMask 浏览器插件**。
    * **📱 手机端**：限定使用 **MetaMask App 内置浏览器** 原生沙箱环境操作。
    * *(若在手机外部浏览器点击连接，系统会自动通过 Deep Link 唤醒并导航至 MetaMask App 内打开)*。

### 3. 领取 0G Galileo 官方测试币 (Faucet)
* **深空抽卡与放置采矿**：完全**由服务器代付 Gas（玩家完全免费、零弹窗）**。
* **拍卖行挂售与购买藏品**：拍卖行智能合约皆在 0G 链上以 **0G 原生代币** 进行买卖结算与合约授权。
* **免费领币步骤**：
  1. 前往 **[0G 官方水龙头 (Faucet)](https://faucet.0g.ai/)**。
  2. 粘贴您的钱包地址并完成验证。
  3. 点击领取，每日可免费获得 **0.1 0G** 测试代币（足够进行数十次拍卖行挂售、购买与授权操作！）。

### 4. 0G Galileo 网络参数 (若钱包未自动切换可手动填入)
...
* **智能预先授权检查**：上架时自动检查链上授权，已授权项目自动跳过 Approve 步骤，省时又省 Gas。
* **标准原生 Web3 Provider 通讯（彻底告别脆弱的 Mobile SDK）**：全面采用 EIP-1193 标准 Provider 与 EIP-3326 平滑换链，避免第三方 SDK 在自定义链上产生的通讯黑盒子与超时卡死，保证 0G 交易结算 100% 稳定。
